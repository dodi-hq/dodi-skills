...
- The driver runs a drive loop: select by the priority table, execute the selected lane playbook **inline** — dispatching each phase worker as its own leaf, never a nested lane subagent (0.14.1 interim; per the leaf-worker dispatch contract a nested lane strands at its first dispatch) — close out, re-select — advancing as many actions as fit before **park** (no automated action possible), **refresh-park** (planned, count-based context refresh), or **bloat** (context degraded). The paused `pickup-next` fallback is still one-action-per-tick.
