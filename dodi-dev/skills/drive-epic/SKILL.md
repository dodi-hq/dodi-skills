...
2. **Driver-claim check** (`${CLAUDE_PLUGIN_ROOT}/scripts/driver-claim.sh status <epic-id>`).
   ...
   - **Pending-capacity wake edge:** an epic labeled `pending-capacity` (a fable capacity park — AGENTS.md § Fable Availability Policy) *is* a wake candidate, unlike a pending-human park. Run a minimal one-shot fable **probe** dispatch (smallest possible prompt, e.g. "reply DONE"); probe success → clear `pending-capacity`, treat the epic as actionable (acquire + Boot; the booted driver retries the blocked dispatch first), probe failure → exit still parked (re-probed next guard tick). Persistent capacity flapping is counted by the standard retry ceiling → `blocked` + escalation, so the epic never loops park↔probe forever.
...
1. **Select** by the mode-parameterized priority table in `epic-orchestrator/state-transitions.md` (§ Workflow Mode and Driver Priority Table — single canon for the full ordering and eligibility). Read the epic's `mode-sprint`/`mode-waterfall` label each loop pass (piggybacking the re-scan — the register's latest `Kind: MODE` entry is truth if the label is ever stale), and **re-evaluate the mode at each lane close-out and the boot audit** (the driver's own inline judgment over the coupling graph it already holds — no separate dispatch; a flip lands a new `Kind: MODE` entry, per `state-transitions.md` § Workflow Mode), to pick the lane-slot order: sprint interleaves deliver→mature, waterfall runs mature-all-then-deliver-all (deliver ineligible until every unblocked child carries `ready-to-implement`); the top four slots (merge → coherence review → epic PR → resume RESUMABLE) are mode-independent. Same demotion rules and `coherence-pending` blocking scope from that table — **which includes the merge slot: no merge action is eligible while `coherence-pending` is set** (only a `RESUMABLE` deliver-lane resume is exempt, per the table). This keeps reviews serial and the register append-ordered; the set-difference audit is the producer-independent backstop.
   ...
   - **Coherence-review action** targets every merged-but-unregistered SHA, **oldest `mergedAt` first, serially, halting after the first pending-human verdict completes its own routing.** Definitions (*pending-human*, *unresolved*, *clean*, *halt*) are in AGENTS.md / the state table and are referenced, not restated. The label clears iff the set-difference (over coherence-verdict entries only — those with no `Kind:` field) is empty ∧ **no register entry over the epic's merged SHAs is unresolved** — register-wide, never batch-scoped; an unresolved pending-human entry gates further remediation, the boot queue, and the guard's actionable-work test until its `RULING` lands (written out-of-band by the human's `rule-coherence` session). Zero merged child PRs clears vacuously.
...
- Record per session: run id, guard outcome, boot audit result, actions taken, exit (`parked`/`refresh-park`/`bloat-handoff`/`no-op`/`ruled`/`taken-over`/`error` — the release-state enum), continuation-brief link, driver-claim release state.
