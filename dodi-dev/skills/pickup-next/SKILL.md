...
2. **Select one action**, highest priority first, honoring the routing tables and demotion rules in `epic-orchestrator/state-transitions.md`:
   ...
   5. Dispatch `deliver-ticket` for a child that passes `${CLAUDE_PLUGIN_ROOT}/scripts/dispatch-eligible.sh` (readiness labels ∧ no open blocking relations ∧ epic signed off and not `coherence-pending`); use `epic-orchestrator/lane-dispatch-prompt.md`. **Blocked for a `coherence-pending` epic.**
