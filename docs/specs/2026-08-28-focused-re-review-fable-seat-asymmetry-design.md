...
Two structurally identical post-fix focused re-reviews were tiered differently (`review/SKILL.md:47` at the gate's fable seat, `:55` pinned `model: opus`) while the governing inherit rule (`AGENTS.md:72`) contradicted the shipped `:55` pin, so the mandatory dispatch-time fable-policy lookup (`execution-model.md:15`) had no deterministic answer — the only two ledger occurrences ever closed at different tiers. **The operator has ruled (DR-025, 2026-08-29Z): Direction B scoped to standard-tier tickets** — the child-PR post-fix re-round downgrades to Capable (`opus@high`) on standard-tier tickets (a plain seat: no substitution, no make-up), keeps its **hard** fable seat on `needs-capable-delivery` tickets, and the verify-stage re-review stays Capable on every tier; no hard seat moves, no park edge changes, and the fable bill drops one deferred dispatch per standard-tier child-PR fix loop. This revision drafts the exact replacement text for `AGENTS.md`, `review/SKILL.md:47–:48` (`:48`'s gate-clean rule now names the re-round as the fix-loop closer), and `child-pr-integration-prompt.md`, plus the validator pins and the DR-003 register-hygiene record — implementation is mechanical; Key Points carry the detail.

## Key Points

- **The ruling (DR-025, operator, 2026-08-29Z):** child-PR post-fix re-round (`review/SKILL.md:47`) → Capable tier (`model: opus`) on **standard-tier tickets**; unchanged **hard** fable seat on `needs-capable-delivery` tickets. Verify-stage focused re-review (`:55`) → unchanged, Capable on **any** ticket tier. Direction A rejected as unnecessary added cost; full Direction B rejected because an all-`opus` re-round on `needs-capable-delivery` tickets would violate writer-≠-final-gate (`AGENTS.md:57`) on exactly the invariant-dense work that tier protects.
...
- **Surfaces that change:** `AGENTS.md` (hard-row Gates cell + the `:72` doctrine sentence — the tier-conditional clause the ruling's scope note requires), `review/SKILL.md:47` (re-round seat) **and `:48`** (gate-clean closure: the fix-loop path's closing round is the re-round at its tier-conditional seat — § Design 2), `child-pr-integration-prompt.md:3,6,10,16` (standard-tier path only), `scripts/validate-phase-skills.sh` (pins). **Unchanged:** `review/SKILL.md:55`, `review-prompt.md`, all orchestrator surfaces, park/make-up machinery, `hook-require-model-pin.sh`.
...
Posted by the operator 2026-08-29Z (UTC — the date the register entry must carry) on DOD-1218; canon as DR-025 on epic DOD-1213. **Direction: B, scoped to standard-tier tickets only** (hybrid of B and the status quo):
...
**`:48` (step 5 — gate-clean closure semantics) is amended, not left unchanged.** The live text ties gate-clean to "the child-PR final round" with the deferred substitution and make-up on standard tier. Under the old doctrine that covered the fix-loop path too, because the re-round inherited the final's seat — it *was* a fresh final. DR-025 breaks that identity on standard tier: a re-round-closed standard-tier gate closes clean at plain `opus@high` with no make-up (§ 4), so with `:48` byte-unchanged the gate-clean rule would be ambiguous on exactly the fix-loop path this ticket governs — the same non-deterministic-lookup defect class the ticket exists to eliminate. One-clause amendment naming the re-round as the fix-loop closer:
...
The final's own seat and policy are untouched on both tiers — the no-fix-loop sentence is the live `:48` policy parenthetical verbatim in substance; only who closes a fix-loop gate changed. This keeps `:48` readable alone: a reader of `:48` gets the make-up answer for both closure paths without consulting § 4 of this spec. `:55` is unchanged byte-for-byte.
...
- **Positive pins:**
  ...
  - (e) `review/SKILL.md` contains the amended `:48` closure clause's fix-loop fragment ("it is the **focused re-round** at its tier-conditional seat") — pinning the gate-clean rule's two-path shape so the fix-loop closer cannot silently revert to final-only.
- **Negative assertions (the pre-ruling shapes cannot reappear):**
  - fail if `review/SKILL.md` contains the retired phrase `focused re-round at the gate's fable seat` (the unconditional inherit-seat wording). Combined with (c), the asymmetry cannot silently reappear without the doctrine sentence being deleted first — and deleting the doctrine sentence itself breaks the build via (c); verified during implementation by deleting it in a scratch copy and confirming non-zero exit;
  - fail if `AGENTS.md` contains the retired phrase `inherit their gate's policy` (the replaced `:72` inherit rule) — so a re-added contradictory inherit sentence alongside the new doctrine sentence also breaks the build, symmetric with the `review/SKILL.md` assertion.
...
- No change to the pre-PR final (`review/SKILL.md:35`), the child-PR integration pair (`:42–:46`) or the **final's** seat and deferred/hard policy on either tier (the `:48` amendment in § Design 2 changes who closes a fix-loop gate, never the final's own seat, substitution, or make-up on the no-fix-loop path), the ledger/catch grammar (`:62–:81`), `review-prompt.md`, the verify-stage surfaces (`state-transitions.md:33`, `deliver-playbook.md`), the park/make-up machinery, or `hook-require-model-pin.sh`.
...
The ticket's acceptance criteria are adopted with the fork resolved: `grep -n "focused re-round at the gate's fable seat" dodi-dev/skills/review/SKILL.md` comes back **empty**; the tier-conditional `:47` clause, the amended `:48` closure clause, the unchanged `:55` clause, the AGENTS.md doctrine sentence, and the hard-row cell addition are all validator-pinned (§ 6). Testing contract: validator wording pins with the negative assertion, metadata lockstep to 0.17.4, full three-script battery, and the manual read-through — a reader with only `AGENTS.md` § Fable Availability Policy + `execution-model.md` § 2 states the pin for both post-fix re-rounds on a standard-tier and a `needs-capable-delivery` ticket (four answers), plus — from `review/SKILL.md:48` alone — whether a re-round-closed standard-tier gate queues a make-up (it does not), recorded in the PR body. No new test harness; no shell-script behavior changes beyond the pins.
