...
- **Decision — the shared 3-day staleness window explicitly does not govern this class.** The wake edge probes hourly (`drive-epic/SKILL.md:37`; cadence stated at `:17` and `:127`); three days is ~72 failed self-corrections. The other six digest classes wait on a human; this one waits on a cap. The sub-section says so in so many words.
...
> - A non-zero exit from `capacity-park-scan.sh` escalates as a script-failure item — it is never rendered as "no parks" or silently skipped; the sweep cannot tell a clean-none from a read failure without the exit code.
