...
- ⚠ **Setup is a product surface:** `setup-dodi-dev` verifies the runtime, plugin, auth, hooks, marketplace, Slack route, and scheduled tasks. Static profile and renewable health are generation-bound; one append-only Linear runtime register is the authoritative escalation ledger; Slack is the sole 0.17 delivery adapter.
...
```json
{
  ...
  "auth": {"linear_source": "env:LINEAR_API_KEY", "linear_runtime_id": "...", "linear_register_issue_id": "...", "github_host": "github.com"},
...
Renewable operational evidence lives separately at `${XDG_CONFIG_HOME:-$HOME/.config}/dodi-dev/runtime-health.json` (or beside an explicit `DODI_RUNTIME_PROFILE`, with `.health.json` suffix). It is mode `0600` and atomically replaced, but it is a projection rather than the authority. Its header carries `schema_version`, the profile's `setup_run_id`, the SHA-256 of the exact static profile bytes, and the Linear register cursor (`comment_id`, monotonic sequence, record hash, projection hash). Minimum projection fields are adapter/channel, last attempt/success timestamps, consecutive failures, and an escalation-obligation map keyed by durable event id. Each obligation records ticket/event identity, `pending | retrying | delivered`, attempts, last error, last-attempt timestamp, and the successful message id/link/time when delivered. The static profile defines the adapter and health policy; the health projection accelerates routing.

The authority is one dedicated Linear runtime-register issue per operator-chosen stable `linear_runtime_id`. Issue creation atomically writes a complete `RUNTIME_INIT` genesis record in the description (`schema_version`, runtime id, created-at, genesis hash); it is not a second comment write. Subsequent append-only typed comments form a hash chain from that genesis with monotonic sequence, previous-record hash, event id, state, and timestamp. Comment species are `ESCALATION_OBLIGATION` and `ESCALATION_DELIVERED`; the plugin never edits the genesis or historical records. Setup searches active and archived issues for the exact runtime id and requires exactly zero or one match; duplicates or a chain fork are blockers. It stores the unique issue id in the profile. Before first initialization it queries Linear: an existing register with valid genesis is replayed, while zero matches permits one issue-create mutation carrying the complete genesis after explicit confirmation. If creation succeeds but its response is lost, restart re-runs the exact-id search, finds and validates that genesis, and continues without a second register. Therefore deletion of both local files cannot masquerade as first install.

The plugin-owned updater holds the stable local lock while it reads the remote register tip and local projection. Before a new obligation is sent, it appends `ESCALATION_OBLIGATION` to Linear and only then projects it locally. After Slack returns a durable message id/link, it appends `ESCALATION_DELIVERED` and then marks the local obligation delivered. A crash at either local-update boundary is repaired by replay. A crash after Slack delivery but before the delivered record may retry and duplicate a notification, but it cannot lose the obligation or falsely mark it resolved. Linear write/read failure blocks the send or replay and leaves the scheduled task failed for the harness-native failure notification.

Profile and health replacement uses a generation-binding protocol. One stable lock file outside the replaceable pair (`runtime-state.lock` in the profile directory, or `<DODI_RUNTIME_PROFILE>.lock` for an explicit path) serializes setup, rollback, Slack, and janitor health writes. Setup disables new scheduled starts, waits for both running tasks and in-process health updaters to quiesce, acquires that lock, and writes the complete prior profile-health pair plus task configuration to a mode-`0700` transaction directory beside the lock; the snapshot and directory are flushed before any live rename. While holding the lock, setup replays the Linear register through its current hash-chain tip, stages and validates both new files, projects every unresolved obligation under the new generation, computes the final profile and projection hashes, flushes both, then atomically renames profile followed by health. A crash between renames leaves a detectable mismatch and cannot unlock a lane. Consumers require `setup_run_id`, profile hash, projection hash, and current Linear register tip to agree before reading health. Rollback restores the prior static profile/tasks but replays the current register into a newly bound health projection so no obligation written after the snapshot can be lost. Crash-point, register-replay, obligation-preservation, and concurrent-updater tests cover every snapshot, lock, remote-write, and rename boundary.

If health is missing, malformed, generation-mismatched, ahead of/forked from the Linear register, or has a projection-hash failure, ordinary runtime paths return `SETUP_REQUIRED`; they never trust or empty-recreate it. Setup reconstructs health only by validating the issue-description genesis and replaying the complete comment chain through the current tip. A valid same-generation projection that is merely a strict prefix of that chain may be replayed by the updater under lock before selection. A missing register/genesis, chain gap/hash failure, duplicate sequence, runtime-id mismatch, or unreadable tip keeps tasks disabled and requires restoration of the Linear register history. A stale same-generation local backup is therefore detected and cannot unlock lanes before replay.

All mutable health updates go through one plugin-owned updater that acquires the same stable external lock across remote-tip read, binding/chain validation, projection mutation, flush, and atomic replacement. Setup creates the initial projection from the register. The Slack adapter and janitor invoke that updater rather than replacing the file independently, preventing concurrent lost updates or writes across setup generations.
...
The profile is invalidated when any of these differ from live state: schema version, installed plugin id/version/root, marketplace root, Codex runtime version, model-catalog fingerprint, resolved hook hash/trust, Linear runtime/register identity, repository remote/base branch, normalized branch-protection/rules/check fingerprints, automation actor/bypass/update-base/rules-administration posture, scheduled-task identity/configuration fingerprints, successful wake-test identity, or configured Slack adapter/channel/policy. Invalidation returns `SETUP_REQUIRED` and performs no workflow write; only `setup-dodi-dev` repairs the profile. Slack delivery age and register cursor movement do **not** invalidate the static profile; they invalidate or refresh the health projection.
...
Health is routed rather than treated as static profile invalidation. Missing, malformed, generation-mismatched, or chain-invalid health returns `SETUP_REQUIRED` and allows no janitor action. A valid generation-bound projection whose cursor merely trails a valid remote tip is replayed under the lock before selection; a projection that is time-stale or delivery-degraded blocks the driver from starting a new lane and queues an adapter test/escalation repair. `notification-degraded` is derived from unresolved obligations after their retry window, not stored as an independently mutable global flag. With valid binding and chain evidence, the janitor remains eligible to repair each pending obligation and perform its read-only waiting-on-you sweep, but performs no destructive cleanup or unrelated PM transition until the adapter is current and no degraded obligation remains. Successful delivery resolves only its matching register obligation; it cannot clear another item's failure.
...
1. Before spawn, require a mutable worker's worktree to be clean and append a `dispatch-intent` containing a unique nonce, owning session/context id, worktree, purpose/write scope, semantic tier, requested model/reasoning, baseline HEAD, baseline porcelain/status hash, and timestamp; flush it before invoking the runtime. Read-only intents record the same baseline but declare no write scope.
...
| terminal digest is present but effective model/reasoning/context attestation is absent or violates gate policy | persist the complete result as `attestation-invalid` with requested/observed fields, then close/reap and compare HEAD + tree/status to the intent baseline | never consume the digest or advance PM/git state; exit `TIER_UNVERIFIED`; if any mutable output exists, quarantine that worktree and allow a successor only in a new worktree created from the recorded baseline commit; unchanged/read-only worktrees may be released after close/reap proof |
...
```json
{"runtime":"codex","session_id":"...","context_id":"...","dispatch_nonce":"...","worktree":"...","purpose":"...","write_scope":"mutable","baseline_head":"...","baseline_status_sha256":"...","tier":"capable","requested_model":"gpt-5.5","requested_reasoning":"xhigh","state":"dispatch-intent","ts":"..."}
...
The sole 0.17 escalation adapter is the installed Slack plugin targeting one configured dedicated channel. Setup verifies the Slack plugin is enabled for the scheduled-task runtime, sends a low-risk test through a registered obligation, records static channel/policy in the profile, appends the delivered evidence to the Linear register, and projects it into health. No configured/tested Slack route means manual workflows remain available and lights-out operation stays disabled.

Before sending, the adapter appends or replays that event's `ESCALATION_OBLIGATION` register record. It sends the artifact's TL;DR + Key Points + links and appends `ESCALATION_DELIVERED` only when Slack returns a durable message id/link. Delivery retries at 0, 30, and 120 seconds. Exhaustion leaves that obligation unresolved, writes a `notification-degraded` comment on the affected ticket naming its event id, causes the scheduled task to exit failed so the harness-native task-failure notification fires, and leaves the item queued for the next janitor run. Open human-wait items are re-posted when their own last successful escalation is older than 24 hours; the new obligation references the prior message id and current age. A later success resolves only its matching obligation. These per-item retry, fallback, evidence, stale re-escalation, register-replay, and derived-global-health semantics are deterministic and adapter-owned.
...
- Validate runtime-profile/health generation binding, Linear register record/hash-chain schemas, and Codex model-map schemas plus runtime-attested fresh-context/model-diversity rules.
- Test bootstrap-without-profile, atomic register genesis with lost create response/pre-comment crash recovery, unique register initialization/replay, post-install local-pair deletion, stale same-generation projection restoration, chain gap/fork/hash failure, every two-file/remote-write crash point, unresolved-obligation carry-forward, missing/mismatched-health `SETUP_REQUIRED`, setup rollback/quiescence, and every profile drift invalidation field/boundary.
- Test mixed Claude/Codex manifest classification and reaping.
- Test authoritative spawn rejection versus ambiguous acceptance, pre-spawn intent crashes, id binding/recovery, unresolvable-intent quarantine, attestation-invalid baseline comparison/worktree replacement, and per-item escalation recovery without cross-item clearing.
...
- one read-only Linear query plus disposable runtime-register obligation/delivery/replay cycle through the resolved environment;
...
| C3 — Codex worker lifecycle adapter | Native spawn/wait/close normalization, manifest schema, pre-spawn baselines, reaping/quarantine, takeover safety, cross-session live gate | C1 | capable |
| C4 — setup/preflight + auth/scheduling/Gate 2/escalation migration | Operator setup skill, atomic runtime-profile writer, Linear runtime register, GitHub auth/protection checks, hook trust/live-fire, automations, escalation adapter, stale marketplace detection | C2, C3 | capable |
...
7. No crash, planned reset, takeover, or attestation failure can leave possibly-writing or wrong-tier Codex output beside a successor writer; unresolved intents and changed attestation-invalid worktrees are quarantined, and successors start from the recorded clean baseline.
8. Setup bootstraps from no profile, resolves or creates one unique append-only Linear runtime register, writes one generation-bound non-secret runtime profile/health projection, detects every specified drift/chain/pair mismatch at its revalidation boundary, and can prove or clearly block the restricted GitHub automation actor, base-branch rules, both hooks, effective-model attestation, per-item Slack escalation, and both scheduled tasks without exposing secrets.
...
- ⚠ Setup may create one dedicated append-only Linear runtime-register issue per operator-chosen runtime id; it is the authoritative escalation ledger and health is a rebuildable local projection.
