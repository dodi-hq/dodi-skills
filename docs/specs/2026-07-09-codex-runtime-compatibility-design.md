...
The authority is one dedicated Linear runtime-register issue per operator-chosen stable `linear_runtime_id`. Its append-only typed comments form a hash chain with monotonic sequence, previous-record hash, event id, state, and timestamp. Record species are `RUNTIME_INIT`, `ESCALATION_OBLIGATION`, and `ESCALATION_DELIVERED`; historical records are never edited or deleted by the plugin. Setup searches active and archived issues for the exact runtime id and requires exactly zero or one match; duplicates or a chain fork are blockers. It stores the unique issue id in the profile. Before first initialization it queries Linear: an existing register is replayed, while zero matches permits creation of one register plus `RUNTIME_INIT` only after explicit confirmation. Therefore deletion of both local files cannot masquerade as first install.
...
If health is missing, malformed, generation-mismatched, ahead of/forked from the Linear register, or has a projection-hash failure, ordinary runtime paths return `SETUP_REQUIRED`; they never trust or empty-recreate it. Setup reconstructs health only by replaying a valid complete register chain from `RUNTIME_INIT` through the current tip. A valid same-generation projection that is merely a strict prefix of that chain may be replayed by the updater under lock before selection. A missing register, chain gap/hash failure, duplicate sequence, runtime-id mismatch, or unreadable tip keeps tasks disabled and requires restoration of the Linear register history. A stale same-generation local backup is therefore detected and cannot unlock lanes before replay.
...
- Test bootstrap-without-profile, unique register initialization/replay, post-install local-pair deletion, stale same-generation projection restoration, chain gap/fork/hash failure, every two-file/remote-write crash point, unresolved-obligation carry-forward, missing/mismatched-health `SETUP_REQUIRED`, setup rollback/quiescence, and every profile drift invalidation field/boundary.
