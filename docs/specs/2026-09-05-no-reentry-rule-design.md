...
- **Decision — enforcement by function hook on Claude Code, doctrine everywhere.** The hook recognises a subagent address the way the runtime resolver does: an agent-id-shaped target (`a` + optional scope + 16 hex, raw or folded) is routed only to a subagent, so it is denied outright; a name this session spawned is denied by exact fold or unique prefix. No manifest lookup, and **no live-registry lookup** — amended at the Frontier review final (2026-09-05): the registry evicts a finished worker ~30 s after completion while a `SendMessage` to its id still resumes it from the transcript on disk, so `$.agent.list()` misses exactly the parked case. Grok Build and Codex get the prose rule only. **Precondition:** in 2.1.261 hooks modules load only when `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` is set or the harness's own rollout flag is on; both are off on this machine today, and when off the loader skips the module with a debug-only line. The doctrine records the flag as a hive-machine setting, and the only proof the guard is live is a live deny.
...
- **Decision — no model-typable escape hatch; address matching mirrors the runtime resolver.** Unnamed dodi-dev workers are addressable only by agent id, so the id-shape check is the load-bearing one, and it holds across eviction, `--resume`, and hot reload because it consults nothing. Named spawns are matched by the resolver's own normalization and by prefix, as the resolver does. The trailing `[ref]` token is a hash the hook cannot reproduce and is stripped, not matched. The only override is operator-side (disable the module).
- **Decision — the module makes no runtime calls at all.** It is a pure function of the address and the names it observed at `agent.spawn`: nothing to fail open from, nothing that can throw out of the hook and unload it. Flag off, an older loader, or a crashed hooks worker leave the rule prose-only; the live verification in § 4 is how a machine proves the guard is on.
...
- **Risk and ⚠ assumptions:** the function-hook API is EARLY ACCESS and undocumented; the hook is kept to one `agent.spawn` observer and one `tool.call` matcher with zero `$` calls, and the test's `$` stub records any access so a surface dependency creeping in is a red test. ⚠ In-process teammates are never denied — their addresses are names, never id-shaped, and their spawn path does not fire `agent.spawn` (read from the binary; plan: live check confirms). ⚠ A fork's runtime-allocated name does not reach `e.name` at `agent.spawn`, so a fork is denied by id only (dodi-dev spawns no forks).
...
3. On Claude Code with the module loaded, a `SendMessage` to a session-spawned subagent is refused by the harness with a reason that names the alternative, under every address form the resolver routes to that subagent — including a worker the registry has already evicted.
4. The rule is testable offline (no runtime dependency), checked by the existing validator, and its live state on a machine is provable.
...
**Behaviour.** The module is a pure function of the target address and the names it observed at `agent.spawn`. It makes no calls on `$`.

```js
const REF_SUFFIX = /^(.*\S)\s*\[[^\]]+\]$/
const AGENT_ID = /^a(?:[\w-]{1,63}-)?[0-9a-f]{16}$/     // the resolver's id shape

export function normalize(s) { /* NFKC, strip Cc/Cf except whitespace, trim, lower, \s+ → "-" */ }
export function stripRef(s)  { /* "worker [3fa9c1]" → "worker" */ }
export function isAgentId(s) { return AGENT_ID.test(s) || AGENT_ID.test(normalize(s)) }

export const register = (on) => {
  const spawnedNames = new Set()
  on("agent.spawn", ($, e, next) => { if (e.name) spawnedNames.add(normalize(e.name)); return next(e) })
  on("tool.call", { tool: "SendMessage" }, ($, e, next) => {
    const raw = String(e.to ?? "").trim()
    if (!raw) return next(e)
    const name = stripRef(raw)
    const n = normalize(name)
    if (n === "main") return next(e)                       // any fold of main is the lead or a peer, never a subagent
    ...
    return { deny: `dodi-dev no-re-entry rule: "${raw}" is a subagent this session dispatched. …` }
  })
}
```

**Why the id shape carries the load — and why the live registry does not.** Read from the 2.1.261 binary at the Frontier review final:

- The resolver tests the id shape first (`Lb(i) ?? Lb(fold(i))`, `Lb = /^a(?:[\w-]{1,63}-)?[0-9a-f]{16}$/`), before any teammate, session, or `main` branch, and `registerName` refuses id-shaped names. So an id-shaped `to` the model can type can only ever reach a subagent: denying every one has zero false positives against deliverable targets. (A teammate's internal resumable id is also id-shaped, but it is never model-visible — the spawn result and `ListAgents` print `name@team` and `name [ref]` — and dodi-dev spawns no teams.)
- A completed background subagent is **evicted from the live task registry ~30 s after its completion notification is consumed** (`evictAfter = now + 30000` unless the task is retained). `$.agent.list()` reads that registry, so it forgets a finished worker exactly when a dispatcher would be tempted to re-enter it. The resolver, however, still routes the id: a missing task resolves as `agent-evicted`, and the send path **resumes the agent from its transcript on disk** (`subagents/agent-<id>.jsonl`, "Resuming agent …"). The same applies after `--resume` (empty registry, transcripts present) and after a hot reload. A registry-based check therefore allows the exact re-entry the rule exists to prevent; the shape check does not.
- The Agent-tool spawn path registers a name only when the call passes `name`. dodi-dev prompts never do, so every dodi-dev worker is addressable by id alone. The name path exists so a named spawn cannot be used as a bypass; it is served by the `agent.spawn` observer, not by the registry.

Helpers, mirroring the runtime resolver as read from the 2.1.261 binary (if the resolver differs, the resolver wins):
...
- Prefix: the resolver matches a typed name to a registered one by prefix only when the typed name is at least 3 characters and exactly one registered name matches; any fold of `main`, bare or with a `[ref]`, is routed to the lead or a peer session by the resolver's exact-fold pass over its candidate index (which always carries `main`), and that pass runs before prefix matching; `registerName` refuses every name that folds to `main`, so no subagent is reachable by any spelling of it. The hook applies the same three rules and passes any fold of `main` through before matching.
...
- `agent.spawn` fires for every Agent-tool spawn with `name` set only when the call passed one; a fork's runtime-allocated name never arrives there. A hot reload of the plugin re-runs `register` and drops the `Set`; the id-shape check is unaffected.
- The matcher form `{ tool: "SendMessage" }` narrows `e` to the tool's arguments spread beside `tool` and `tool_use_id`; `SendMessage`'s target argument is `to` (confirmed in this build's schema). The extracted `claude-code.d.ts` has an empty `BuiltinToolInputs` table (it is filled by `/plugin-types` per session); the plan regenerates the types with `/plugin-types` at the live run. The shipped module has exactly the two hooks and zero `$` calls test 6 asserts.
- The harness skips a hook that throws and runs what is beneath it, and unloads a module that crashes the hooks worker. The module has no async work and no `$` calls, so neither can happen from the fail path — there is no fail path.

**No escape hatch.** Unlike `DODI_ALLOW_UNPINNED`, there is no env or prompt token that lifts this deny: an escape the model can type is not an escape, and the address matching covers every form the resolver routes to a subagent, evicted or not. An operator who needs a resident worker removes the `hooks` entry from the Claude manifest or ships a deliberate agent definition — out of scope here.
...
1. `SendMessage` to an id-shaped address (`a` + 16 hex, and the scoped form `a<scope>-` + 16 hex) ⇒ `deny` mentioning the address, with no registry consulted; case-folded ids ⇒ `deny`; a 15-hex or wrong-letter string ⇒ allow.
...
4. (retired with the registry lookup — there is no runtime call to fail.)
5. Empty `to` ⇒ `next(e)`.
6. The module registers exactly two hooks: `agent.spawn` (unmatched) and `tool.call` with matcher `{ tool: "SendMessage" }`; across the whole run the recording stub `$` saw **no** property access at all.
7. Address forms, with `worker` and `code reviewer` spawned (recorded as `worker`, `code-reviewer`): `worker [3fa9c1]` ⇒ `deny`; `work` and `work [3fa9c1]` (prefix) ⇒ `deny`; `Worker`, `Code  Reviewer`, and `code reviewer` (normalization) ⇒ `deny`; `codereviewer`, `main`, a teammate-style name, and `other [3fa9c1]` ⇒ `next(e)`.
...
**Manual, plan-time:** `claude plugin validate` on the plugin directory shows the scan result for the module (expected: hooks `agent.spawn`, `tool.call`; no calls on `$`). `/plugin-types` regenerates `claude-code.d.ts` with the build's real `SendMessage` input type. With `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` set, `claude --debug` must not print `hooks modules not loaded`, and the live checks in § Verification must pass, including the teammate and fork checks for the two ⚠ assumptions.
...
- **Peer sessions, teammates, `main`.** Never id-shaped and never recorded at `agent.spawn` (teammates are a distinct task type whose spawn path does not fire the event); allowed. A named subagent that shares a peer session's bare name is denied (subagent match wins). Acceptable: names are the dispatcher's own choice, and the runtime itself routes a bare name to the in-session subagent first.
- **Completed, evicted, resumed, or running workers.** All denied: the id shape is the address in every state, and the live registry is never consulted (it evicts a finished worker after ~30 s while the id still resumes it from disk). A running worker is not a re-entry target either — under the leaf rule the dispatcher awaits its digest.
- **Plugin-spawned agents** (`$.agent.spawn` from another plugin) carry the same id shape and are denied as targets too. No dodi-dev flow messages them.
...
- **Hot reload** and **`--resume`** drop `spawnedNames` and empty the live registry respectively; id-shape denial continues in both. Names are only the addressable form for named spawns, which dodi-dev prompts do not use today.
- **Address forms.** Bare name, `name [ref]`, unique name prefix, case and whitespace variants, raw id, folded id, and scoped id all resolve to the same subagent at the runtime and are all denied (§ 3 helpers, tests 1 and 7).
...
- ⚠ In-process teammates are never denied — read from the 2.1.261 binary: their addresses are names (never id-shaped) and the Agent tool's teammate branch never fires `agent.spawn`; the plan's live check confirms rather than discovers.
...
- `bash dodi-dev/scripts/tests/test-hooks-module.sh` passes every case (the groups above plus the review-added id-shape, id-fold, and prefix cases; 34 checks at 0.20.0).
...
- Live, with `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` and the updated plugin: `claude --debug` shows no `hooks modules not loaded` line; `claude plugin validate` lists the module's hooks and calls as § 4 expects; dispatch a Standard-tier agent, let it finish, **wait more than 30 s and confirm `ListAgents` no longer lists it**, then `SendMessage` it by id — refused with the rule text (this is the evicted case the registry would have missed); a named spawn by name prefix — refused; `SendMessage` to a peer session and to a teammate succeed.
