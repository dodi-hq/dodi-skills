...
- The resolver tests the id shape first (`Lb(i) ?? Lb(fold(i))`, `Lb = /^a(?:[\w-]{1,63}-)?[0-9a-f]{16}$/`), before any teammate, session, or `main` branch, and `registerName` refuses id-shaped names. So an id-shaped `to` can only ever reach a subagent: denying every one has zero false positives against deliverable targets.
