...
- **`epic-orchestrator/SKILL.md`:** line 65's stale "dispatch a deliver-ticket lane using lane-dispatch-prompt.md" is replaced with inline execution per the playbook; the contract-table row (line 21) already says inline and its substance stays, **but its "(0.14.1 leaf-worker contract — never nested lane subagents)" parenthetical is reworded to the permanent framing** (e.g. "(per `execution-model.md` — never nested lane subagents)") — the Verification Contract's bare-`0.14.1` grep names this exact line, so leaving it verbatim fails the spec's own gate. **Also in scope:** the § Parallel Lanes section (lines 71–76, "dispatch up to `maxParallelLanes` … `deliver-ticket` lanes concurrently") and the `maxParallelLanes` input (line 32) are deleted — they instruct exactly what `execution-model.md` forbids (concurrent nested lane subagents); one-lane-in-flight is the shipped policy, and the parallelism invariants live in `execution-model.md` §7 for the future flip.
...
All four surfaces carrying a 0.16.0 promise are resolved, but not uniformly. Two carry "the 0.16.0 flatten redesign owns the durable architecture" (`deliver-ticket:57`, `mature-ticket:28`) — both replaced by pointers to `execution-model.md`. `lane-dispatch-prompt:3` carries a different promise ("Retained as input to the 0.16.0 flatten redesign") and dies with its file. **`AGENTS.md:49` carries a different promise** ("parallel lanes return with the 0.16.0 flatten redesign") that this spec does *not* fulfill — parallel dispatch is explicitly out of scope (§ Out of scope) — so its fix is a reword deferring the promise, not a bare pointer: e.g. "parallel lane dispatch remains a future release; `execution-model.md` §7 pins the isolation invariants it must preserve."
...
| **deferred** | opus substitutes now; fable make-up queued as a durable obligation, batched at the dedicated make-up round in submit-epic-pr | standard-tier child-PR final round ⚠ (the paired half of the capable-tier hard proposal above — one linked decision, split across buckets); pre-PR final round (all tiers — the child-PR gate still guards the merge, hard on capable tier) ⚠; plan writing ⚠; **final** plan-review round ⚠ |
...
- **Full reference sweep — six surfaces** (the epic's "three references" undercounted; these are the grep-verified set):
  ...
  5. `drive-epic/SKILL.md` §Scheduling migration — compressed to a completed-history note, **worded without literal `0.14.x` version strings** (e.g. "the interim scheduled-tick migration is complete; pickup-next is deleted") so the bare-`0.14.1` grep-clean gate passes on this ship surface;
...
- Grep-clean: no remaining bare `0.14.1`, `lane-dispatch-prompt`, `pickup-next`, or `maxParallelLanes` references outside historical docs (`docs/plans/`, `docs/specs/`, and `docs/handoffs/` are historical and exempt — the 2026-07-03 maintainer handoff carries two live `pickup-next` mentions that stay as history; the bare-`0.14.1` pattern deliberately catches phrasings like "0.14.1 leaf-worker contract" that the narrower `0.14.1 interim` misses, e.g. `epic-orchestrator/SKILL.md:21`, whose reword C1 now carries as an explicit change item).
