...
- **Doctrine amendment (explicit change items — three surfaces).** The shipped "park and bloat — the **only two exits** / no third exit trigger" doctrine (`drive-epic/SKILL.md:81-83`) and its echo "an actual context reset happens only at park or bloat" (`AGENTS.md:96`, `epic-orchestrator/SKILL.md:97`) are amended, not merely contradicted: the exits become **park** (no automated action possible — including the pending-human and new pending-capacity species), **refresh-park** (planned, count-based, only at a post-merge durable seam), and **bloat** (emergency). The original prohibition's substance is retargeted to what it always meant and keeps verbatim: **no time- or silence-based exit or succession trigger** — refresh-park is count-based at a durable boundary, not a clock rule. All three surfaces are updated in the same child.
...
5. **C4** — refresh budget + `refresh-park` exit state + seven-state enum sync + three-surface exit-doctrine amendment. Blocked by C1b.
